...
std::vector<FullMove> Piece::getMoves(BoardPieceInterface const &board, int pieceRow, int pieceCol, bool attackingMoves) const {
    return getMovesImplementation(board, pieceRow, pieceCol, attackingMoves);
}

FullMove Piece::createFullMove(BoardPieceInterface const &board, int fromRow, int fromCol, int toRow, int toCol, int captureRow, int captureCol, MoveType moveType, bool isAttackingMove, PieceType promotionPieceType) const {
    Piece const &capturePiece = board.getPieceAt(captureRow, captureCol);
    return FullMove(fromRow, fromCol, toRow, toCol, captureRow, captureCol, 
